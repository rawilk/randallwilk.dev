...
export default {
    ...
    computed: {
        /**
         * Generate the backdrop CSS classes.
         *
         * @returns {array}
         */
        backdropClasses () {
        ...
        /**
         * Generate the dialog CSS classes.
         *
         * @returns {array}
         */
        dialogClasses () {
        ...
        /**
         * Generate the footer CSS classes.
         *
         * @returns {array}
         */
        footerClasses () {
        ...
        /**
         * Generate the header CSS classes.
         *
         * @returns {array}
         */
        headerClasses () {
        ...
        /**
         * Generate the modal CSS classes.
         *
         * @returns {array}
         */
        modalClasses () {
    ...
    mounted () {
        ...
        this.listenOnRoot('lara::show::modal', this.showHandler);
        this.listenOnRoot('lara::hide::modal', this.hideHandler);

        // Listen for lara:modal::show events, and close ourselves if the opening modal not us
        this.listenOnRoot('lara::modal::show', this.modalListener);
    ...
    methods: {
        ...
        hide (trigger) {
            ...
            const hideEvent = new VueLaravelEvent('hide', {
                ...
                cancel () {
                    ...
                    warn(
                        'b-modal: event.cancel() is deprecated. Please use event.preventDefault().'
                    );
            ...
            if (hideEvent.defaultPrevented || !this.is_visible) {
                return;
            }
        ...
        show () {
            ...
            if (showEvent.defaultPrevented || this.is_visible) {
            ...
            if (hasClass(document.body, 'modal-open')) {
                ...
                this.$root.$once('lara::modal::hidden', this.doShow);
            } else {
                // Show the modal
                this.doShow();
            }
        ...
        /**
         * Handle before enter transition.
         */
        onBeforeEnter () {
        ...
        /**
         * Handle before leave transition.
         */
        onBeforeLeave () {
            this.is_transitioning = true;
            this.setResizeEvent(false);
        ...
        emitEvent (laraVueEvent) {
            ...
            this.$emit(type, laraVueEvent);

            this.$root.$emit(`lara::modal::${type}`, laraVueEvent);
        },

        /**
         * Handle clickout event.
         *
        ...
        onEsc (event) {
            ...
            if (event.keyCode === KeyCodes.ESC && this.is_visible && ! this.noCloseOnEsc) {
                this.hide('esc');
            }
        },
        ...
        onFocusout (event) {
            ...
            if (! this.noEnforceFocus && this.is_visible && content && ! content.contains(event.relatedTarget)) {
                content.focus();
            }
        },
        ...
        setResizeEvent (on) {
            ['resize', 'orientationchange'].forEach(event => {
                if (on) {
                    eventOn(window, event, this.adjustDialog);
                } else {
                    eventOff(window, event, this.adjustDialog);
                }
            });
        },
        ...
        modalListener (laraVueEvent) {
            // If another modal opens, close this one
            if (laraVueEvent.vueTarget !== this) {
                this.hide();
            }
        },
