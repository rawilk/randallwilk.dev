...
function applyLARATT (el, bindings, vnode) {
    if (! inBrowser) {
        return;
    }
    ...
    if (el[LARATT]) {
        el[LARATT].updateConfig(parseBindings(bindings));
    } else {
        el[LARATT] = new ToolTip(el, parseBindings(bindings), vnode.context.$root);
    }
}

// Remove tooltip on our element
function removeLARATT (el) {
    if (! inBrowser) {
        return;
    }
