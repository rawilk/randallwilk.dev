...
const ACTIVATE_EVENT = 'lara::scrollspy::activate';
...
class ScrollSpy {
    ...
    setObservers (on) {
        ...
        if (on) {
            this.$obs_targets = observeDom(this.$el, () => {
            ...
            this.$obs_scroller = observeDom(this.getScroller(), () => {
                this.handleEvent('mutation');
            }, {
                subtree: true,
                childList: true,
    ...
    refresh () {
        ...
        selectAll(this.$selector, this.$el)
            ...
            .map(href => {
                ...
                if (isVisible(el)) {
                    return {
                        offsetTop: parseInt(methodFn(el).top, 10) + offsetBase,
                        target: href
                    };
                }
                return null;
            ...
            .reduce((memo, item) => {
                ...
                if (! memo[item.target]) {
                    this.$offsets.push(item.offsetTop);
    ...
    process () {
        const scrollTop = this.getScrollTop() + this.$config.offsetTop;
        const scrollHeight = this.getScrollHeight();
        const maxScroll = this.$config.offsetTop + scrollHeight - this.getOffsetHeight();
